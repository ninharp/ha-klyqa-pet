...
async def test_fan_commands(hass: HomeAssistant, mock_purifier: MagicMock) -> None:
    await hass.services.async_call(FAN_DOMAIN, SERVICE_TURN_OFF, {ATTR_ENTITY_ID: ENTITY_ID}, blocking=True)
    mock_purifier.set_power.assert_awaited_with(False)

    await hass.services.async_call(
        FAN_DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: ENTITY_ID, ATTR_PERCENTAGE: 100}, blocking=True
    )
    mock_purifier.set_power.assert_awaited_with(True)
    mock_purifier.set_fan_level.assert_awaited_with(3)

    await hass.services.async_call(
        FAN_DOMAIN, SERVICE_SET_PERCENTAGE, {ATTR_ENTITY_ID: ENTITY_ID, ATTR_PERCENTAGE: 33}, blocking=True
    )
    mock_purifier.set_fan_level.assert_awaited_with(1)

    await hass.services.async_call(
        FAN_DOMAIN, SERVICE_SET_PERCENTAGE, {ATTR_ENTITY_ID: ENTITY_ID, ATTR_PERCENTAGE: 0}, blocking=True
    )
    mock_purifier.set_power.assert_awaited_with(False)

    await hass.services.async_call(
        FAN_DOMAIN, SERVICE_SET_PRESET_MODE, {ATTR_ENTITY_ID: ENTITY_ID, ATTR_PRESET_MODE: "pet"}, blocking=True
    )
